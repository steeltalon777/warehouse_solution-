...
- Редактировать и закрывать issues; менять любые поля проекта кроме `Status`; добавлять/удалять items; ставить labels/assignees/milestones; создавать/закрывать/мержить PR; переводить задачу в `Done` без проверенных критериев приёмки.
- Исполнителям (build, swarm, cheap_sub) запрещено создавать issues и менять статусы — они только читают доску/issues и докладывают результат.

### Кто может создавать задачи и менять статусы

- **Архитектор и ревьюер** могут создавать новые issues (полное описание проблемы: симптом, репозиторий/компонент, критерии приёмки) и менять `Status` по таблице ниже.
- Остальные агенты, даже работая по TZ и конкретным указаниям, обязаны уметь **прочитать** задачу (см. «Обязательная процедура чтения»).
...
**Точка входа для любого агента в любой сессии:** owner `steeltalon777`, проект `1`, репозитории — из «Данные проекта» выше.

1. Найти issue: на доске `gh project item-list 1 --owner steeltalon777 --limit 100 --format json` или в репозитории `gh issue list --repo OWNER/REPO --state open --json number,title,state`.
2. Прочитать **тело** issue через

...
- `docs/TZ-AGENT_ROLE_ADMIN_UI.md` - ✅ реализовано (2026-08-07): Django admin теперь позволяет назначить role=agent (LLM-агент) с пустыми сайтами и `default_site_id=null`. `Role.AGENT` в TextChoices и choices обоих CharField, `MANAGED_ROLE_CHOICES` включает «LLM-агент», `clean()` ослаблен для agent, `services.build_scopes` возвращает `[]` для пустых site_ids, `default_site_id` явно сериализуется как `None`. Добавлен helper `is_agent` в `apps/common/permissions.py`. Миграция `users/0014_alter_syncuserbinding_sync_role_and_more`. 22 новых теста в `apps/users/tests/test_agent_role_admin.py` (A–G). Stand smoke: agent успешно создан через admin, SyncServer принял role=agent + scopes=[], status=synced. ADR-0030, SyncServer TZ-AGENT-ROLE-SYNCSERVER rev.2.

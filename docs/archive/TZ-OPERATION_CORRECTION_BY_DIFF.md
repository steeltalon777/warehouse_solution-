...
- [x] 11. Service: _validate_delta (safe-delete matrix V1) — ✅ REV.3 VERIFIED 2026-07-27: live 422 correction_new_item_invalid (not_found); item_replaced full old_qty (код)
- [x] 12. Service: submit_correction (atomic, immutable revision create, lock order) — ✅ REV.3 VERIFIED 2026-07-27 live: submit 200 (revision 2, effects +5, locks работают); idempotent retry 200 (без 500/422)
- [x] 13. Document: OperationRevisionLine вместо OperationLine (INV-C16) — ✅ REV.3 VERIFIED 2026-07-27 live: новый документ 4e68cb1f rev=2, payload qty=110 (из revision lines), reuse обойдён при operation_revision_id
- [x] 14. Document: operation_revision_id FK + supersede chain — ✅ FINAL VERIFIED 2026-07-27 live: цепочка двусторонняя — новый doc 5d815ca5 rev=3 finalized с `supersedes_document_id` → 4e68cb1f (superseded) → d750c585 (superseded)
- [x] 15. Endpoints: begin + PUT + POST/PATCH/DELETE line + submit + DELETE abandon + GET draft + root-only auth — ✅ REV.3 VERIFIED 2026-07-27 live: PUT с baseline uuids 200; GET draft 200
- [x] 16. Audit events — ✅ REV.3 VERIFIED 2026-07-27 live: operation.correction.applied + document.revision_created + document.superseded все пишутся
...
- [x] 18. Unit tests: 22 теста (mock-based) pass; полный прогон: 656 passed, 3 skipped, 7 xfailed, 13 deselected (stand) — rev.3 перепроверено 2026-07-27, регрессий нет
- [x] 19. Integration tests — ✅ FINAL VERIFIED 2026-07-27: 6/6 pass (11s) через общие conftest-фикстуры (ephemeral DB); покрывают happy path, V1 scope, kind-reject, baseline clone, duplicate uuid, failed-validation-draft
- [x] 20. Concurrency tests: 5 unit tests (version conflicts, status transitions, abandon+submit) — REVIEW note: mock-based; partial unique + version conflict проверены live (409)
- [x] 21. Documentation: ARCHITECTURE.md (data model + correction flow + principles), Functional and WorkLogik.md (section II.6.8.1)
- [x] 22. Final acceptance review: 🔴 REJECTED rev.1 (2026-07-24) → 🔴 REJECTED rev.2 (2026-07-24) → 🟡 ACCEPTED WITH NOTES rev.3 (2026-07-27) → 🟢 ACCEPTED final (2026-07-27): V1 flow end-to-end live (begin→PUT→submit→revision→documents+supersedes chain→audit→idempotency), integration 6/6, full suite 662 passed. Единственный остаток: retry subset-response (deferred пользователем как не-блокер, задокументировано)

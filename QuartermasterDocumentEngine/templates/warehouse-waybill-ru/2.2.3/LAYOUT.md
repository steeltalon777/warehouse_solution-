# warehouse-waybill-ru@2.2.3 — LAYOUT

**Bounded basis/table spacing prototype; not promoted.** Copied from frozen
2.2.2. The only rendering change is `v(3mm, weak: false)` after the requisites
inside `title_unit(true, config)`. Typst trims the last nested block's weak
`below` spacing, and its last line ends at the baseline: the old table stroke
crossed the basis glyphs. Explicit in-flow spacing is measured and occupied.

HF increases by 3 mm (8.503937 pt); A_first and A_single decrease by the same
amount. HM, HT, HS, A_middle and A_last are unchanged. Pagination, item rows,
nullable fallbacks, signatures and layout-config are byte-identical to 2.2.2.
Changing main.typ is necessary because the missing terminal flow spacing is
defined there; no layout-config or signature change is needed.

The physical regression requires >= 2 mm clear space between the lowest
basis glyph ink and the painted upper edge of the 0.6 pt table stroke.
Three mm from the baseline covers the observed 0.8072 mm glyph descender,
0.1058 mm half-stroke and 2 mm clearance (2.087 mm measured on anchor32).
This is a relative ink-to-stroke invariant, not an absolute page-coordinate
snapshot. Acceptance of any partition changes remains a separate review gate.

The remaining sections document the inherited 2.2.2 model and its historical
evidence. They are not a claim of 2.2.3 production acceptance or new goldens.

...
**Date:** 2026-07-08 (rev. 2: architect review; rev. 3: коррекция draft-карты; rev. 4: plan B; rev. 5: first_max 23→22; rev. 6: renderer cache key; rev. 7: content-aware physical-row units; rev. 7.1: first-page packing + layout cache namespace)
...
|---|---|---|---|
| 🔴 rev.7/7.1 | User (real PDF evidence) | Fixed 22/28 logical-row caps still failed when item names wrapped: operation `1/0331/090726` had 2 logical pages but 3 physical PDF pages (page 2 only contained `Кладовщик` and `Лист 1 из 2`). **Root cause:** 22/28 are physical visual-line capacities, not unconditional item counts; also the paginator reserved the last page before filling the first. **Fix:** word-aware visual units (40 chars baseline), first=22 units, middle=28 units, conservative final-form capacities, static footer; fill first prefix before reserving final tail. Docker WeasyPrint smoke: `logical_pages=2`, `physical_pages=2`, `logical_rows=[21,1]`, no signature-only page. Layout cache namespace `layout-v7.1` forces re-render independently from `DOCUMENT_RENDERER_VERSION`. | Stage I1/I2/I4 |
...
| 🔴 rev.6 | User (storekeeper bug report) | **HOTFIX: cache_key не включал renderer_version.** `services.py:73` использовал только `{document_id}:{payload_hash}`, поэтому бамп `DOCUMENT_RENDERER_VERSION` v1→v2→v3 не инвалидировал кэш для старых накладных. **Воспроизведено 09.07.2026:** кладовщик открыл operation `11673bc0-...`, нажал «сформировать накладную», получил PDF v1 (старый формат, без first_max=22). **Fix:** cache_key = `waybill_pdf:{doc_id}:{hash}:{renderer_version}:{template_version}`. +2 guard-теста. Дополнительно: после deploy в prod рекомендуется `python manage.py shell -c "from django.core.cache import cache; cache.clear()"` для очистки осиротевших ключей. | `Warehouse_web/apps/documents/services.py:73-83` |

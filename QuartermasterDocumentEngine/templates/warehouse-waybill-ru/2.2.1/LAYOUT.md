# warehouse-waybill-ru@2.2.1 — LAYOUT
...
package**: the contract allows explicit JSON null for the nested object
fields `receiver` / `sender` / `operation` / `basis`, and the template
now guards them (`type(...) == dictionary`) instead of crashing on
`none.at(...)`. Rendering and page counts are byte-identical to 2.2.0
on every payload 2.2.0 could render
...
production; superseded by 2.2.0, in turn patched by 2.2.1). Form
...
* `2.2.0` — measurable pagination rebalance (frozen; superseded by
  the 2.2.1 patch).
* `2.2.1` — ADR-0034 null-safety patch over 2.2.0 (this version,
  current mapped production version in `DOCUMENT_TEMPLATE_MAP`);
  rendering/layout unchanged, goldens under
  `tests/golden/warehouse-waybill-ru-2.2.1/`.

...
- Закрывать issues; менять любые поля проекта кроме `Status`; добавлять/удалять items; ставить labels/assignees/milestones; создавать/закрывать/мержить PR; переводить задачу в `Done` без проверенных критериев приёмки.
- Исполнителям (build, swarm, cheap_sub) запрещено создавать/редактировать issues и менять статусы. Доску они читают только по прямому указанию или когда задача явно ссылается на issue — без этого контекст не засоряют. Прочитать задачу при этом умеют:
  `gh issue view N --repo OWNER/REPO --json number,title,body,state,assignees,labels,comments,url` (выводы из одного заголовка не делать).

### Кто может создавать и менять задачи

- **Архитектор и ревьюер** могут создавать и редактировать issues (полное описание проблемы: симптом, репозиторий/компонент, критерии приёмки) и менять `Status` по таблице ниже.
- Остальные агенты работают по TZ и конкретным указаниям; задачи на доске они не трогают.
...
| `make test-e2e-headed` | Запустить Playwright в headed-режиме локально |
...
## Prod Work (prod_working)

Если пользователь просит поработать с продом (API, VPS, БД) — сначала читай `prod_working/AGENT_INSTRUCTIONS.md`.

- Все не-readonly операции на проде — только с явного подтверждения пользователя.
- Операции создавать черновиками; мержи без команды не проводить.

// warehouse-waybill-ru@2.2.3 — canonical warehouse waybill in Typst.
// 2.2.3: reserve an explicit basis/table gap inside the measured title unit.
//
// 2.2.1 is a null-safety patch over the immutable 2.2.0 package
// (ADR-0034): the contract warehouse.operation-document/v2 allows
// explicit JSON null for the nested object fields receiver / sender /
// operation / basis / recipient, but ``.at(...)`` on ``none`` is a
// Typst compile error. Rendering and layout are byte-identical to
// 2.2.0 on every payload 2.2.0 could render.
//
...
#let operation_display_number() = {
  ...
  // Nullable per contract: explicit JSON null must not crash (2.2.1).
  let op = inner.at("operation", default: none)
  if type(op) == dictionary {
  ...
  let site_id = "0"
  let sender = inner.at("sender", default: none)
  if type(sender) == dictionary {
    let sv = sender.at("site_id", default: none)
    if sv != none {
      site_id = str(sv)
    }
  }
...
#let consignee_label() = {
  ...
  // Nullable per contract (historical RECEIVE payloads carry
  // receiver: null): fall through to recipient/sender exactly like
  // the legacy _consignee_label chain (2.2.1, ADR-0034).
  let receiver = inner.at("receiver", default: none)
  if type(receiver) == dictionary {
...
#let basis_label() = {
  ...
  let basis = inner.at("basis", default: none)
  if type(basis) == dictionary {
...
#let title_unit(is_first, config) = block(above: 0pt, below: 0pt)[
    ...
    [
      ...
      // A trailing block's weak below-spacing is trimmed at the unit edge.
      // Keep this gap in HF (and therefore A_first/A_single), not outside it.
      #v(3mm, weak: false)
    ]
  } else {
    block(spacing: 4mm)[

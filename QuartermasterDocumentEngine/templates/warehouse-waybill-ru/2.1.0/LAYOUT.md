# warehouse-waybill-ru@2.1.0 — LAYOUT

Canonical production waybill template (Typst). Derived from
`warehouse-waybill-ru@2.0.0` (the faithful port of the frozen legacy
Django/WeasyPrint form, `Warehouse_web@133e2fa`).

**2.1.0 is a customer-driven form change (03.09.2026) and
intentionally diverges from the legacy renderer:**

* signatures are rendered on the **last page only** — no short
  storekeeper line on first/middle pages;
* the storekeeper signature is always present on the last page for
  every operation type;
* per-operation-type signature sets: MOVE → `Водитель` + `Груз
  принял`; ISSUE / ISSUE_RETURN / EXPENSE / WRITE_OFF → `Имущество
  принял на списание` + `Списание разрешил`; RECEIVE / ADJUSTMENT /
  CORRECTION → storekeeper only;
...
* Table.
* No signature content (2.1.0; 2.0.0 had a short `Кладовщик` line).
* Capacity: `23` row units (see §5).
...
* Table.
* No signature content (2.1.0).
* Capacity: `29` row units.

## 4. Last page

* Short header.
* Table with the remaining rows.
* Full signature form (2.1.0 sets — see §7):
  * `Кладовщик: _________________/__________________` — always, for
    every operation type;
  * MOVE: `Водитель` (driver-style) + `Груз принял`;
  * ISSUE / ISSUE_RETURN / EXPENSE / WRITE_OFF: `Имущество принял на
    списание` + `Списание разрешил`;
...
* Capacity: `22` row units for MOVE (per-operation-type values in
  config).
...
* Roles and capacities (visual units, 2.1.0):

| Role | Page header | Signature block | Capacity (MOVE) |
|---|---|---|---|
| `first` | full | none | `23` |
| `middle` | short | none | `29` |
| `last` | short | full form | `22` |
| `single` | full | full form | `18` |

  ISSUE-family operation types (ISSUE / ISSUE_RETURN / EXPENSE /
  WRITE_OFF): `last` = `23`, `single` = `17`. Default (storekeeper
  only): `last` = `26`, `single` = `21`.
...
full capacity (e.g. 23 rows on the first page, 29 on a middle page)
consumes almost the whole 267 mm frame (rows are 8.5 mm/unit exactly
like the legacy budget; the body — header/thead, and on last pages
the signature form — fits the frame; verified for 2.0.0: body bottom
≤ 802.2 pt on the pathological input; 2.1.0 removes the per-page
short storekeeper, freeing ~12.5 mm on first/middle pages). The sheet
counter is NOT part of the body flow (it lives in the reserved footer
area), so it cannot overhang or create an orphan page even here; the
frozen legacy renderer produced an orphan counter page on the same
input (a documented legacy defect class that rev.7 was designed to
avoid) — QDE intentionally does not reproduce it. Real production
data (mixed line lengths) never hits this case. Capacities are NOT
reduced for this case — that would change the accepted allocation
...
* `components/signatures.typ` renders signatures from declarative
  ...
  template.
* 2.1.0: signatures render on the LAST page only. The storekeeper
  line opens the full form on the last page (every operation type);
  first/middle pages carry no signature content. (2.0.0 rendered a
  short storekeeper line on every page — dropped by the customer form
  change 03.09.2026.)
* Block flavours:
...
`layout-config.typ.operation-signature-sets`. The 2.1.0 sets (customer
form change 03.09.2026):

| operation type | key | label | flavour |
|---|---|---|---|
...
form and is not part of the sets. Payload fields are intentionally NOT
invented: the form uses static placeholders, so blocks carry no
payload binding.
...
* `2.0.0` is the accepted canonical production version (frozen; keeps
  rendering byte-identically).
* `2.1.0` is the customer form change of 03.09.2026 (signatures on
  the last page only; new per-type signature sets; re-calibrated
  capacities). Goldens for 2.1.0 are produced after the customer
  accepts the printed form.
* Customer form changes → new semver version (patch/minor/major at
  the customer's discretion): `2.2.0`, `3.0.0`, …
